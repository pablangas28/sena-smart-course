...
async function cargarCursos() {
  loading.value = true
  try {
    cursos.value = (await apiFetch('/cursos')).filter(c => c.estado === 'activo')
  } catch {
    snackbar.text = 'No se pudieron cargar los cursos.'; snackbar.color = 'error'; snackbar.show = true
  } finally {
    loading.value = false
  }
}
...
async function generarFormulario() {
  ...
  try {
    ...
    snackbar.text = 'No se pudo generar el formulario.'; snackbar.color = 'error'; snackbar.show = true
  } finally {
...
function copiarLink() {
  ...
  snackbar.text = '¡Link copiado al portapapeles!'; snackbar.color = 'success'; snackbar.show = true
}

const cursoSelNombre = computed(() => cursos.value.find(c => c.id === cursoSel.value)?.nombre ?? '')
</script>
...
<template>
  <div class="pa-6" style="max-width:640px">
    ...
    <v-card rounded="xl" elevation="0" class="pa-6 form-card">
      ...
      <!-- Info -->
      <div class="d-flex align-start ga-3 pa-4 rounded-xl mb-5" style="background:rgba(123,31,162,0.06); border:1px dashed rgba(123,31,162,0.25)">
        <v-icon color="#7B1FA2" class="mt-1 flex-shrink-0">mdi-information-outline</v-icon>
        <p class="text-body-2 text-grey-darken-1">
          El link generado es público. Cualquiera que lo tenga puede inscribirse en el curso.
          Compártelo por WhatsApp o correo con los aprendices.
        </p>
      </div>

      <div v-if="loading" class="text-center pa-8">
        <v-progress-circular indeterminate color="#7B1FA2" />
      </div>
      ...
      <template v-else>
        ...
        <v-text-field
          ...
          density="comfortable"
          bg-color="white"
          hint="Si no se define, el formulario no expira."
          class="mb-5"
        />
    ...
    <v-dialog v-model="dialogLink" max-width="520">
      <v-card rounded="xl" class="pa-6">
        <div class="d-flex align-center ga-3 mb-5">
          <v-avatar color="success" size="52"><v-icon color="white" size="28">mdi-check-circle</v-icon></v-avatar>
          <div>
        ...
        <v-card color="rgba(123,31,162,0.06)" rounded="lg" class="pa-4 mb-5">
        ...
        <div class="d-flex ga-3">
          <v-btn color="#7B1FA2" rounded="lg" prepend-icon="mdi-content-copy" @click="copiarLink">
            Copiar link
          </v-btn>
