...
async function cargarTodo() {
  ...
  try {
    ...
    // Cargar estudiantes de todos los cursos en paralelo
...
async function cambiarEstado(reg, nuevoEstado) {
  try {
    ...
    snackbar.text = `Estado actualizado a ${nuevoEstado}.`; snackbar.color = 'success'; snackbar.show = true
  } catch {
    snackbar.text = 'No se pudo cambiar el estado.'; snackbar.color = 'error'; snackbar.show = true
  }
}
</script>
...
<template>
  <div class="pa-6">
    <div class="d-flex align-center justify-space-between flex-wrap ga-3 mb-6">
      ...
      <v-chip variant="tonal" color="#7B1FA2" size="small">
        {{ estudiantesFiltrados.length }} Aprendices
      </v-chip>
    </div>
    ...
    <div class="d-flex ga-3 flex-wrap align-center mb-5">
      ...
      <v-select
        ...
        variant="outlined" rounded="lg" density="compact"
        hide-details bg-color="white" style="max-width:240px"
      />
    </div>
    ...
    <div v-else-if="estudiantesFiltrados.length === 0" class="text-center pa-12">
      <v-icon size="64" color="grey-black-1">mdi-account-off-outline</v-icon>
      <p class="text-body-1 text-black mt-3">
        {{ estudiantes.length === 0 ? 'Aún no hay aprendices inscritos en tus cursos.' : 'No se encontraron resultados.' }}
      </p>
    </div>
          ...
          <tr v-for="est in estudiantesFiltrados" :key="est.id" class="est-row">
            <td>
              <div class="d-flex align-center ga-3 py-2">
                <v-avatar color="#7B1FA2" size="36" class="text-white text-body-2 font-weight-bold flex-shrink-0">
            ...
            <td class="text-body-2 d-none d-sm-table-cell">
              <nuxt-link :to="`/dashboard/aliado/cursos/${est.cursoId}`" class="text-decoration-none" style="color:#7B1FA2">
                {{ est.cursoNombre }}
              </nuxt-link>
            </td>
            <td>
              <v-chip :color="estadoColor[est.estado]" size="x-small" rounded="lg" class="font-weight-bold text-uppercase">
                {{ est.estado }}
              </v-chip>
            </td>
            <td class="text-center">
              <v-menu>
                <template #activator="{ props }">
                  <v-btn v-bind="props" size="x-small" variant="tonal" color="#7B1FA2" rounded="lg">
...
.est-row:hover { background: rgba(123,31,162,0.04) !important; }
</style>