...
const estudiantes  = ref([])   // registros del curso
const asistencias  = ref([])   // asistencias ya guardadas
const calificaciones = ref([]) // calificaciones ya guardadas
...
async function cargarTodo() {
  ...
  try {
    ...
    // Mapear asistencias existentes por estudiante_id
    const asistMap = {}
    asistData.forEach(a => { asistMap[a.estudiante_id] = a })

    // Mapear calificaciones existentes por estudiante_id
    const califMap = {}
...
async function guardarAsistencia() {
  ...
  try {
    await apiFetch(`/clases/${claseId}/asistencia`, {
      method: 'POST',
      body: {
        asistencias: asistencias.value.map(a => ({
    ...
    mostrarSnackbar('Asistencia guardada correctamente.')
  } catch {
    mostrarSnackbar('No se pudo guardar la asistencia.', 'error')
  } finally {
...
async function guardarCalificaciones() {
  ...
  try {
    ...
    if (items.length === 0) {
      errorCalif.value = 'Ingresa al menos una nota antes de guardar.'
      return
    }
    ...
    mostrarSnackbar('Calificaciones guardadas correctamente.')
  } catch {
    mostrarSnackbar('No se pudo guardar las calificaciones.', 'error')
  } finally {
...
const porcentajeAsist = computed(() => {
  if (!asistencias.value.length) return 0
  return Math.round((totalAsistieron.value / asistencias.value.length) * 100)
})
...
<template>
  <div>
    ...
    <v-alert v-else-if="error" type="error" variant="tonal" class="ma-6" rounded="xl">
      {{ error }}
    </v-alert>
    ...
    <template v-else>
      ...
      <div class="clase-hero pa-6 pb-0">
        ...
        <div class="d-flex align-center ga-2 mb-4 text-body-2 text-grey flex-wrap">
          <nuxt-link to="/dashboard/instructor" class="text-grey text-decoration-none">Mis Cursos</nuxt-link>
          <v-icon size="14">mdi-chevron-right</v-icon>
          <nuxt-link :to="`/dashboard/instructor/cursos/${cursoId}`" class="text-grey text-decoration-none">
            Curso
          </nuxt-link>
          <v-icon size="14">mdi-chevron-right</v-icon>
        ...
        <div class="d-flex align-start justify-space-between flex-wrap ga-3 mb-5">
          <div>
            <div class="d-flex align-center ga-2 mb-1">
              <v-chip
                :color="clase?.tipo === 'presencial' ? 'blue' : 'purple'"
                size="small"
                rounded="lg"
              >
                {{ clase?.tipo }}
            ...
            <div class="text-body-2 text-grey">
              {{ formatFechaHora(clase?.fecha_hora) }} · {{ clase?.duracion_horas }}h
            </div>
          </div>
        </div>
        ...
        <div class="d-flex ga-4 pb-4 flex-wrap">
          <div class="stat-mini">
            <span class="font-weight-bold" style="color:#39A900">{{ totalAsistieron }}</span>
        ...
        <!-- Tabs -->
        <v-tabs v-model="tab" color="#39A900" bg-color="transparent">
        ...
        <v-tabs-window v-model="tab">
          ...
          <v-tabs-window-item value="asistencia">
            <div v-if="estudiantes.length === 0" class="text-center pa-12">
              ...
              <p class="text-body-1 text-grey mt-3">No hay estudiantes inscritos en este curso.</p>
            </div>
            ...
            <template v-else>
              ...
              <div class="d-flex align-center ga-3 mb-5">
                <v-progress-linear
                  ...
                  class="flex-grow-1"
                />
                <span class="text-body-2 font-weight-bold" style="min-width:40px">
                  {{ porcentajeAsist }}%
                </span>
              </div>
              ...
              <div class="d-flex ga-2 mb-4">
                <v-btn
                  ...
                  color="success"
                  rounded="lg"
                  @click="asistencias.forEach(a => a.asistio = true)"
                >
                ...
                <v-btn
                  ...
                  color="grey"
                  rounded="lg"
                  @click="asistencias.forEach(a => a.asistio = false)"
                >
                  Limpiar
                </v-btn>
              </div>
              ...
              >
                <div class="d-flex align-center ga-3 flex-wrap">
                  ...
                  <v-switch
                    ...
                    hide-details
                    density="compact"
                    :label="item.asistio ? 'Presente' : 'Ausente'"
                    inset
                  />
                </div>

                <!-- Observación (solo si ausente) -->
                <v-expand-transition>
                  <v-text-field
                    v-if="!item.asistio"
          ...
          <v-tabs-window-item value="calificaciones">
            ...
            <template v-else>
              <p class="text-body-2 text-grey mb-4">
                Ingresa notas de <strong>0.0 a 5.0</strong>. Podés dejar en blanco si no aplica para esa clase.
              </p>

              <v-alert v-if="errorCalif" type="warning" variant="tonal" rounded="lg" class="mb-4" closable>
                {{ errorCalif }}
              </v-alert>

              <v-card
                v-for="item in calificaciones"
                :key="item.estudiante_id"
                rounded="xl"
                elevation="0"
                class="calif-row pa-4 mb-3"
              >
                <div class="d-flex align-center ga-3 flex-wrap">
                  <v-avatar
                    :color="notaColor(item.nota)"
                    size="40"
                    class="text-white font-weight-bold text-body-2"
                  >
              ...
              <v-btn
                ...
                class="mt-4"
                prepend-icon="mdi-content-save"
                :loading="guardandoCalif"
                :disabled="!calificacionesValidas"
...
<style scoped>
.clase-hero {
  background: rgba(200, 215, 200, 0.3);
...
.asist-presente {
  background: rgba(57, 169, 0, 0.08) !important;
  border-color: rgba(57, 169, 0, 0.2) !important;
}
