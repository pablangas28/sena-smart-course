  ...
  <div class="pa-6">
    ...
    <div v-else-if="cursos.length === 0" class="text-center pa-16">
      ...
      <p class="text-h6 font-weight-medium text-grey mt-4">
        No hay cursos {{ filtroEstado !== 'todos' ? filtroEstado + 's' : 'registrados' }}.
      </p>
    </div>
    ...
    <v-row v-else>
      <v-col v-for="curso in cursos" :key="curso.id" cols="12" sm="6" lg="4">
        ...
        >
          ...
          <div class="text-body-2 text-grey-darken-1 mb-4 desc-clamp">
          ...
          <v-progress-linear
            :model-value="porcentaje(curso)"
            color="#1976D2" bg-color="rgba(0,0,0,0.08)"
            rounded height="6"
          />
        </v-card>
      </v-col>
